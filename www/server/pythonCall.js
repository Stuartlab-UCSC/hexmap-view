...
//DCHANGE
function make_local_parm_file (opts, in_json) {
    // Save parameters to a file in json and return the file name as json that
    // looks like this: {"parm_filename": "<filename>"}
    // in_json of true indicates the options are already in json format
    var json_opts = in_json ? opts : JSON.stringify(opts);

    return writeToTempFile(json_opts)
}
...
function call_python_local (pythonCallName, json, context) {
    ...
    console.log('call_python_local: json:', json);
...
exports.call = function (pythonCallName, opts, context) {
    console.log('exports.call: opts:', opts);
    ...
    //TODO: we should have more control over when a remote calc is called than this...
    if (CALC_URL) {
        ...
        if (opts.parm_filename) {
            ...
            parm_filename = make_parm_file(opts); //DCHANGE
        }
        //console.log("calling call_pythonLocal with:", pythonCallName, parm_filename);
